...
def getModel(model_name):
    ...
    with graph.as_default():
        tf.config.init_training_mode()
        net = tf.input_data(shape=[None, n])
        net = tf.fully_connected(net, n, activation='tanh', regularizer='L2')
        net = tf.fully_connected(net, n, activation='tanh', regularizer='L2')
        net = tf.fully_connected(net, n, activation='tanh', regularizer='L2')
        net = tf.regression(net, optimizer='rmsprop', loss='mean_square')
        model = tf.DNN(net)
...
def train(model, minibatch, y, game_number, model_name):
    ...
    model.fit(x, y, n_epoch=3, validation_set=0.01, shuffle=True, run_id=name)
